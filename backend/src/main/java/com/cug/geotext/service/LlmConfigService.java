...
public class LlmConfigService {
    ...
    public void applyProvider(String provider){
        ...
        LlmConfig config=mapper.selectOne(new LambdaQueryWrapper<LlmConfig>().eq(LlmConfig::getProvider,provider.toLowerCase()));
        if(config==null)throw new BusinessException(409,"所选模型尚未保存配置: "+provider);
        apply(config);
    }

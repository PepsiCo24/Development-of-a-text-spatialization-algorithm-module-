...
public class GraphSyncService {
    private static final Set<String> GRAPH_RELATIONS=Set.of("LOCATED_IN","CONTAINS","CONTROLS","INTRUDES");
    ...
    public GraphStatus start(long documentId){GeologicalDocument d=documentService.get(documentId);if(!"COMPLETED".equals(d.getSpatialStatus()))throw new BusinessException(409,"请先完成文本空间化");if("SYNCING".equals(d.getGraphStatus()))throw new BusinessException(409,"图谱同步正在进行中");update(d,"SYNCING",5,null);executor.execute(()->sync(documentId));return status(documentId);}
    ...
    }catch(Exception exception){d=documentMapper.selectById(documentId);if(d!=null){String message=exception instanceof RestClientResponseException r?"图谱服务返回 HTTP "+r.getStatusCode().value():exception.getMessage();update(d,"FAILED",zero(d.getGraphProgress()),abbreviate(message));}}}

    private String nodeType(String type){return switch(type){case "STRATUM"->"STRATUM";case "ROCK_BODY"->"ROCK_BODY";case "FAULT"->"STRUCTURE";case "ORE_BODY","MINERALIZATION_ZONE"->"ORE_BODY";case "MINERAL"->"MINERAL";case "PLACE"->"REGION";default->null;};}

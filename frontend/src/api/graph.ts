...
export interface GraphLink{source:number;target:number;relationType:'LOCATED_IN'|'CONTAINS'|'CONTROLS'|'INTRUDES';confidence?:number;sourceText?:string;page?:number}
...
export async function queryGraphNodes(query=''):Promise<GraphView>{const response=await http.get('/graph/nodes',{params:{query,limit:200}});return response.data.data}

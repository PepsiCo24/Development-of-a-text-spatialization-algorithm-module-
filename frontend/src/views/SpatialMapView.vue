...
function initializeMap() {
  ...
  map = new Map({
    ...
    view: new View({ center: fromLonLat([114.9, 30.1]), zoom: 9, minZoom: 3, maxZoom: 20 }),
...
async function load(){loading.value=true;try{const[page,spatial]=await Promise.all([listDocuments({page:1,size:100,status:'PARSED'}),listSpatialObjects()]);documents.value=page.records;objects.value=spatial;if(!selectedDocumentId.value)selectedDocumentId.value=spatialDocuments.value[0]?.id;if(selectedDocumentId.value)status.value=await getSpatialStatus(selectedDocumentId.value);renderObjects()}catch(e){ElMessage.error(message(e))}finally{loading.value=false}}
...
function fitAll(){if(!map||!vectorSource||vectorSource.isEmpty())return;const extent=vectorSource.getExtent();if(extent&&!isEmptyExtent(extent))map.getView().fit(extent,{padding:mapPadding(),maxZoom:13,duration:450})}
...
function selectObject(id:number){const object=objects.value.find(item=>item.id===id);if(!object)return;selected.value=object;const feature=vectorSource.getFeatures().find(f=>f.get('objectId')===id);if(feature&&map){map.getView().fit(feature.getGeometry()!.getExtent(),{padding:mapPadding(true),maxZoom:15,duration:450});vectorLayer.setStyle(f=>featureStyle(f as Feature<Geometry>,f.get('objectId')===id))}nextTick(()=>document.querySelector(`[data-spatial-id="${id}"]`)?.scrollIntoView({behavior:'smooth',block:'nearest',inline:'center'}))}
